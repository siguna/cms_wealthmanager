...
const navigationReducer = createReducer(
  initialState,
    on(NavigationActions.reloadAllItems, (state, action) => ({
      ...state,
      items: decorateItems(action.items),
      total: countTotal(action.items),
      filterActive: false,
      filterText: '',
      matched: 0
    })),
...
function filterItems(navItems: NavigationItem[], text: string): NavigationItem[] {
  ...
  return navItems.map(navItem => {
    ...
    if(item.matched && item.items && item.items.length>0){
      item.open = true;
    }
    return item;
  });
...
function navItemMatch(item: NavigationItem, text: string) {
  return (!text.trim() || (item.title && !!item.title.match(new RegExp(`.*${text.trim()}.*`, 'gi'))));
